...
class TripForm(forms.ModelForm):
    ...
    class Meta:
        model = Trip
        fields = [
            "title", "start_date", "route", "persons_number", "bike_type",
            "clothes", "repair_kit", "bags", "sleeping_kit", "electronics",
            "toiletries", "cooking_kit", "additional_items"
        ]
        widgets = {
            'start_date': forms.DateInput(attrs={'type': 'date'}),
        }  # Datepicker created thanks to Stackoverflow

    # to not accept start date in the past-credits:https://copyprogramming.com/
